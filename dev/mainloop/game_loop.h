...
// game_loop.h - Da game loop.
	...
	while (playing) {
		#asm
			...
			if (sp_KeyPressed(GRA_MUSICA_EN_JUEGO)) {
                //if (flags[GRA_MUSICA_ID2] == 0) {
				    if (flags[GRA_MUSICA_ID] == 1) {
					    // Activada, hay que desactivarla
					    STOP_SOUND();
					    flags[GRA_MUSICA_ID] = 0;
				    }
				    else {
					    // Desactivada, hay que activarla
					    PLAY_MUSIC(1);
					    flags[GRA_MUSICA_ID] = 1;
				    }
                    //flags[GRA_MUSICA_ID2] == 1;
                    sp_WaitForNoKey();
                //}
			/*} else {
                // No esta la tecla pulsada.
                flags[GRA_MUSICA_ID2] == 0;*/
            }
		#endif
